...
void main() {
    ...
    float radius = 0.17; 
    float dist = roundedBoxSDF(vUvs - 0.5, vec2(0.5), radius);
  ...
  constructor(width = 1, height = 1, widthSegments = 1, heightSegments = 1) {
    ...
    for (let iy = 0; iy < gridY1; iy++) {
      ...
      for (let ix = 0; ix < gridX1; ix++) {
        ...
        this.lastVertex.uv[0] = ix / gridX;
        this.lastVertex.uv[1] = 1 - (iy / gridY);
      }
    }
...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: number | AllowSharedBufferSource | null, usage: number) {
...
class InfiniteGridMenu {
  ...
  constructor(canvas: HTMLCanvasElement, items: any[], onActiveItemChange: (index: number) => void, onMovementChange: (moving: boolean) => void, onInit: ((instance: InfiniteGridMenu) => void) | null = null, scale = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  #init(onInit: ((instance: InfiniteGridMenu) => void) | null) {
    ...
    this.discGeo = new PlaneGeometry(2, 2, 1, 1);
        ...
        this.items.map(
          item =>
            new Promise<HTMLImageElement>(resolve => {
              ...
              const src = isExternal ? `/api/image-proxy?url=${encodeURIComponent(item.image)}` : item.image;
              ...
              img.onerror = (e) => {
                console.error("InfiniteMenu: Image failed to load via proxy", src, e);
                // Try direct load as fallback (might work if CORS was actually fine but proxy failed)
                if (src.includes('/api/image-proxy')) {
                  img.src = item.image;
                } else {
                  resolve(img); // Resolve with broken image to avoid hanging
                }
              };
              img.src = src;
            })
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: { items: Item[]; scale?: number }) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      // console.log("InfiniteMenu: Active item changed", index);
    ...
    if (canvas) {
      try {
        ...
        console.error("InfiniteMenu: Error initializing sketch", e);
      }
    } else {
      console.error("InfiniteMenu: Canvas ref is null");
    }
