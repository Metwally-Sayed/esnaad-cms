  ...
  return (
    <section className="px-4 sm:px-6 md:px-10 py-10 sm:py-12 bg-background text-foreground transition-colors min-h-[85vh]">
      <div className="mx-auto max-w-6xl space-y-4 sm:space-y-5">
        ...
        <ScrollReveal width="100%" className="grid gap-2 sm:gap-3 md:grid-cols-3" staggerChildren={0.1}>
          {cards.map((card, index) => {
             ...
             const containerClass = cn("group relative block overflow-hidden", card.link && "cursor-pointer");

             if (card.link) {
               return (
                 <motion.div key={key} variants={cardVariants} className={containerClass}>
             ...
             if (card.actionType === "button") {
               return (
                 <motion.button
                   ...
                   onClick={() => console.log(`Action clicked for: ${card.title}`)}
                   type="button"
                 >
                   {cardContent}
                 </motion.button>
